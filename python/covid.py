...
datadirectory = Path().cwd().parent / 'csse_covid_19_data' / 'csse_covid_19_time_series'
...
def make_long(df, varname):
    ...
    df_long = pd.melt(df, id_vars=ids, value_vars=dates,
                   var_name='date', value_name=varname)
